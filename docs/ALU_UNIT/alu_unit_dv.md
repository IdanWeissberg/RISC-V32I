...
## 1. Objective
To verify the complete mathematical and logical operation of the Arithmetic Logic Unit (ALU). The plan validates **Functional Correctness** across all defined operations and rigorously tests **Corner Cases** using explicitly defined binary and hexadecimal stimulus vectors.

> **Note:** The `zero` output flag has been removed from this module. Branch condition evaluation is now handled by the dedicated `branch_comparator` unit.
...
* **Expected Output:** `alu_result` = `32'h0000_0096` (150 in Dec)

### TC_02: Basic Arithmetic (SUB)
* **Inputs:** * `alu_op_in` = `4'b0110` (`ALU_CTRL_SUB`)
  * `read_data_1` = `32'h0000_0064` (100)
  * `alu_mux` = `32'h0000_0032` (50)
* **Expected Output:** `alu_result` = `32'h0000_0032` (50)

### TC_03: Bitwise Logic (AND, OR, XOR)
* **Inputs:** `read_data_1` = `32'hAAAA_AAAA`, `alu_mux` = `32'h5555_5555` (Alternating bit masks)
* **Test A (AND):** `alu_op_in` = `4'b0000` -> **Expected:** `alu_result` = `32'h0000_0000`
* **Test B (OR):** `alu_op_in` = `4'b0001` -> **Expected:** `alu_result` = `32'hFFFF_FFFF`
* **Test C (XOR):** `alu_op_in` = `4'b0011` -> **Expected:** `alu_result` = `32'hFFFF_FFFF`

### TC_04: Logical Shifts (SLL, SRL)
* **Inputs:** `read_data_1` = `32'h0000_000F`, `alu_mux` = `32'h0000_0004` (Shift by 4)
* **Test A (SLL):** `alu_op_in` = `4'b0100` -> **Expected:** `alu_result` = `32'h0000_00F0`
* **Test B (SRL):** `alu_op_in` = `4'b0101` -> **Expected:** `alu_result` = `32'h0000_0000`
...
* **Expected Output:** `alu_result` = `32'h1234_5000`
...
* **Expected Output:** `alu_result` = `32'h0000_0002`
...
* **Expected Output:** `alu_result` = `32'h8765_4321`
...
* **Test A (SLT - Signed):** `alu_op_in` = `4'b1000` -> **Expected:** `alu_result` = `32'h0000_0001` (-1 is less than 1)
* **Test B (SLTU - Unsigned):** `alu_op_in` = `4'b1001` -> **Expected:** `alu_result` = `32'h0000_0000` (0xFFFFFFFF is NOT less than 1)
...
* **Expected Output:** `alu_result` = `32'hF800_0000`
* **Description:** Ensures MSB is replicated (`1111_1000...`) instead of zero-filled.

### TC_10: Subtraction Result Zero (Equality Check)
* **Inputs:** * `alu_op_in` = `4'b0110` (`ALU_CTRL_SUB`)
  * `read_data_1` = `32'hABCD_1234`
  * `alu_mux` = `32'hABCD_1234`
* **Expected Output:** `alu_result` = `32'h0000_0000`
